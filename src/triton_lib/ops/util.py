        ...
        for expr in exprs:
            ...
            if any(isinstance(e, tlib.expr.stage3.Concatenation) for e in expr):
                concat_index, concat_expr = [
                    ...
                    if isinstance(e, tlib.expr.stage3.Concatenation)
                ][0]
...
def assignment(exprs_in, exprs_out):
    ...
    axes_out = [
        {a.name for a in tlib.expr.stage3.get_named_axes(expr_out)} for expr_out in exprs_out
    ]
...
def transpose_broadcast(expr_in, tensor, expr_out, *, backend, broadcast=True):
    assert tlib.expr.stage3.is_flat(expr_in) and tlib.expr.stage3.is_flat(expr_out), (
    ...
    if len(out_axes_broadcast) > 0:
        pre_broadcast_shape = tuple(
            ...
            for a in tlib.expr.stage3.get_axes(expr_out)
        )
