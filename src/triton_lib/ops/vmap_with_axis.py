...
def vmap_with_axis_stage3(
    ...
    if all(tlib.tracer.is_scalar(tensor) for tensor in tensors_in):
        raise ValueError(
            "At least one input tensor must be a non-scalar"
        )  # TODO: support this
    kwargs = {**kwargs}

    # Call tensor factories
    tensors_in = [
        tlib.tracer.call_factory(tensor, expr.shape, backend=backend)
    ...
    def is_broadcast_axis(expr):
        return (
            isinstance(expr, tlib.expr.stage3.Axis) and expr.name not in in_axis_names
        )

    exprs_out_flat = util.flatten(exprs_out)
    exprs_out_flat_without_broadcast = [
        tlib.expr.stage3.remove(expr, is_broadcast_axis) for expr in exprs_out_flat
    ]
    ...
    for root in list(exprs_in) + list(exprs_out_flat_without_broadcast):
        for v in root:
            if (v.name in vmapped_axis_names) != is_vmapped(v):
                raise ValueError(
                    f"Axis {v.name} appears both as vmapped and non-vmapped"
                )
    ...
    if transpose_first:
        ...
        marked_input_axes = {
            ...
            and tlib.expr.stage3.is_marked(axis)
        }
        ...
        exprs_op_output = [
            to_op_output(expr_out_flat_wb)
            for expr_out_flat_wb in exprs_out_flat_without_broadcast
        ]
    ...
    tensors_out = [
        ...
        for expr_in, tensor_out, expr_out in zip(
            exprs_op_output, tensors_out, exprs_out_flat
        )
    ]

    # Unflatten output expressions
    tensors_out = util.unflatten(
        exprs_out_flat, tensors_out, exprs_out, backend=backend
    )
